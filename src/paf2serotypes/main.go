...
func main() {
	...
	chunkChan, errorChan, done := pafprocessor.StreamingProcessAllChunks(chunks, mapping, *minAlignmentScore, *paired, *numWorkers, *useRAlgorithm)
	...
	// Debug channel, identical to chunkChan
	debugChan := make(chan []pafprocessor.SummaryEntry, *numWorkers)
	for i := 0; i < *numWorkers; i++ {
		// add the worker to the pool
		resultGroup.Add(1)
		pafprocessor.Logger.Infof("Starting worker %d", i)
		go func() {
			defer resultGroup.Done()
			for chunk := range chunkChan {
				// print lenght of the result
				pafprocessor.Logger.Infof("Result length for %d: %d", i, len(chunk))
				debugChan <- chunk
				if i == 0 {
					pafprocessor.Logger.Infof("Processing worker %d", i)
					for _, result := range chunk {
						// process the result
						pafprocessor.Logger.Infof("%v", result)
					}
				}
			}
			// mark the worker as done
			pafprocessor.Logger.Infof("Worker %d done", i)
	...
	if err := pafprocessor.StreamingGenerateResults(debugChan, *outputDir, *sampleName); err != nil {
	...
	pafprocessor.Logger.Infof("Processing completed in %s", time.Since(startTime))
	// Close the chunk channel
	// resultGroup.Wait()
	// close(debugChan)
	done()
	pafprocessor.Logger.Infof("Processing completed in %s", time.Since(startTime))
